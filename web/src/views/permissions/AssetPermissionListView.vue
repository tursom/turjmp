<template>
  <div class="page-container">
    ...
    <div v-else-if="error" class="page-error">
      <el-result icon="error" title="Failed to load permissions" :sub-title="error">
        <template #extra>
          <el-button type="primary" @click="fetchPermissions">Retry</el-button>
        </template>
      </el-result>
    </div>
    ...
    >
      ...
      <el-table-column prop="actions" label="Actions" min-width="200" />
      <el-table-column label="Date Start" width="160" align="center">
        <template #default="{ row }">
          {{ row.date_start ? formatDate(row.date_start) : '—' }}
        </template>
      </el-table-column>
      <el-table-column label="Date Expired" width="160" align="center">
        <template #default="{ row }">
      ...
      <el-table-column label="Actions" width="160" align="center" fixed="right">
        <template #default="{ row }">
          <el-button v-if="canUpdatePermissions" size="small" @click="goEdit(row.id)">Edit</el-button>
...
async function handleDelete(perm: AssetPermission) {
  try {
    await ElMessageBox.confirm(
      `Delete permission "${perm.name}"? This action cannot be undone.`,
      'Confirm Delete',
  ...
  try {
    ...
    ElMessage.success(`Permission "${perm.name}" deleted`)
